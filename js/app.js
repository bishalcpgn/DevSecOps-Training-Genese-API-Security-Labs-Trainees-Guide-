...
(function () {
  ...
  const THEME_KEY = 'apisec-trainee-theme';
  ...
  const PROGRESS_KEY = 'apisec-trainee-progress';
  ...
  function refreshMarkDoneBtn(id) {
    ...
    if (textEl) textEl.textContent = done ? 'Solved ✓' : 'I solved this';
  }
  ...
  document.addEventListener('click', (e) => {
    if (e.target.closest('#markDoneBtn')) {
      ...
      updateProgressUI();
      updateCertificateUI();
      showToast(`${currentId.toUpperCase()} solved!`);
      return;
    }
  ...
  const LAB_TIMERS_KEY = 'apisec-trainee-lab-timers';
  ...
  function updateCertificateUI() {
    ...
    const navIcon  = document.getElementById('cert-nav-icon');
    const countEl  = document.getElementById('cert-progress-count');
    const count    = [...completed].filter(id => TRACKABLE.has(id)).length;
    const total    = TRACKABLE.size;
  ...
  function setCertDate() {
    const el = document.getElementById('certDateVal');
    if (!el) return;
    const d = new Date();
  ...
  document.addEventListener('click', (e) => {
    if (e.target.closest('#certGenerateBtn')) {
    ...
    if (e.target.closest('#certPrintBtn')) {
      // Ensure the cert wrapper is visible before printing
      const wrapper = document.getElementById('certWrapper');
      if (!wrapper || wrapper.style.display === 'none') {
        showToast('Generate your certificate first.');
        return;
      }
      window.print();
      return;
  ...
  // Also regenerate name live as user types (if cert is already showing)
  document.addEventListener('input', (e) => {
    if (e.target.id === 'certNameInput') {
      const wrapper = document.getElementById('certWrapper');
      if (wrapper && wrapper.style.display !== 'none') {
        const nameDisplay = document.getElementById('certNameDisplay');
        if (nameDisplay) nameDisplay.textContent = e.target.value.trim() || '< Your Name Here >';
      }
    }
  });

  // Initial cert UI state
  updateCertificateUI();
